...
{
	fprintf(stderr, "error: line %d: near: '%s': ", token->line + 1, token->lexeme);
	fprintf(stderr, "%s\n", message);
	parser->has_error = 1;
...
AST_STMT *block(PARSER *parser)
{
	...
	return conditional_stmt(parser);
}
...
AST_STMT *conditional_stmt(PARSER *parser)
{
	if (parser_match(parser, IF))
	{
		LIST *values = init_list(sizeof(AST_STMT *));
		...
		AST_STMT *true_clause = block(parser);
		list_push(values, condition);
		list_push(values, true_clause);
		if (parser_match(parser, ELSE))
		{
			AST_STMT *false_clause = block(parser);
			list_push(values, false_clause);
		}
...
AST_EXPR *equality(PARSER *parser)
{
	...
	while (parser_match(parser, EQUALS) || parser_match(parser, LESS_THAN) || parser_match(parser, GREATER_THAN) || parser_match(parser, LESS_THAN_EQ) || parser_match(parser, GREATER_THAN_EQ))
	{
